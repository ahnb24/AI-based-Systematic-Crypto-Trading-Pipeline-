...
def history_data_stage_one(feature_config, logger=default_logger):
    ...
    for symbol in list(feature_config.keys()):
        ...
        # Find outliers
        outliers = returns[~returns.between(-0.5, 0.5)]

        # Assert and also show outliers if any
        assert outliers.empty, f"pct_change outlier returns detected at indices {outliers.index.tolist()} with values {outliers.tolist()}"
